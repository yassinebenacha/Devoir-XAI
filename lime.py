...
def sample_points(model, dataset, num_points, seed=0):
    ...
    # Get bounds for first two features
    x1_min = hps[labels[0]].lower
    x1_max = hps[labels[0]].upper
    x2_min = hps[labels[1]].lower
    x2_max = hps[labels[1]].upper
    
    np.random.seed(seed)
    # Uniform sampling
    X = np.random.uniform(low=[x1_min, x2_min], high=[x1_max, x2_max], size=(num_points, 2))
...
def weight_points(x_interest, Z, kernel_width=0.2):
    ...
    distances = np.sqrt(np.sum((Z - x_interest)**2, axis=1))
